...
export async function sendConfirmation(
  ...
  const html = `
    <div style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #fff5f6; border-radius: 16px; color: #4a2530;">
      <h1 style="font-size: 22px; margin: 0 0 8px;">
        ${isDeclined ? "Réponse à l'invitation" : "C'est officiel, on a un rendez-vous !"}
      </h1>
      <p style="margin: 0 0 24px; color: #8a5560;">
        ${isDeclined ? "Voici la réponse envoyée par " + userName : "Voici les détails confirmés par " + userName}
      </p>
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      to: RECIPIENTS,
      subject: isDeclined 
        ? `${userName} a répondu : Non merci` 
        : `Rendez-vous confirmé par ${userName} : ${activityLabel}`,
      html,
    })
